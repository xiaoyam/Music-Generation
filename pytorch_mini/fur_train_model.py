...
def compute_corr(y_true, y_pred):
    ...
    return 1-row_corr, 1-col_corr
    

def compute_loss(spect_true, spect_pred, pcm_true, pcm_pred, corr, mode, epoch, total_epochs, l1, l2):
    """
    Returns the mean MSE
    """
    sq_errors= []
    if l1 == 'True' and l2 == 'False':
        spect_errors = torch.abs(spect_pred - spect_true)
    elif l2 == 'True' and l1 == 'False':
        spect_errors = (spect_pred - spect_true) ** 2
    else:
        spect_errors = torch.abs(spect_pred - spect_true) + (spect_pred - spect_true) ** 2
...
def optimize_network(args, model, mask_model, spect, pcm, mode, **kwargs):
    ...
    if mode == 'train':
        ...
        mask_net_optimizer = optim.Adam(mask_model.parameters(), lr = model_lr)
    ...
    while True:
        ...
        for i in range(n_batches):
            ...
            loss, loss_unnorm = compute_loss(spect_true, spect_pred, pcm_true, pcm_pred, corr, norm_mode, epoch, total_epochs, args['l1'], args['l2'])
        ...
        if mode == 'train' and (best_loss == None or avg_unnormed_loss < best_loss):
            ...
            best_epoch = epoch
            best_mask_model = pickle.loads(pickle.dumps(mask_model))
            best_mask_model = best_mask_model.to(torch.device('cpu')) 
    ...
    if mode == 'train':
        ...
        return train_latents, losses, float(best_loss), best_epoch, best_model, best_mask_model
        ...
        with torch.no_grad():
            ...
            for i in range(n_batches):
                ...
                mask = mask_model(latents_use)[0]
                if args['backprop_pcm']:
                    [spect_pred, pcm_pred] = model(latents_use)
                else:
                    res = model(latents_use)
                    spect_pred = res[0]*mask
        ...
        _, final_test_loss = compute_loss(spect, all_pred, pcm, pcm_pred, False, None, epoch, total_epochs, True, False)
...
def write_to_csv(path, args, results):
    if args['l1'] == 'True' and args['l2'] == 'False':
        loss_t = 'L1'
    elif args['l1'] == 'False' and args['l2'] == 'True':
        loss_t = 'L2'
    else:
        loss_t = 'L1 + L2'
    with open(path, 'a') as f:
        f_writer = csv.writer(f, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
        f_writer.writerow([args['config_num'], args['decoder'], args['n_train_epochs'], args['n_test_epochs'], args['model_lr'], args['train_latents_Adam_lr'], args['test_latents_Adam_lr'], args['n_decoder_layers'], args['n_hidden_units'], args['train_batch_size'], args['test_batch_size'], args['backprop_pcm'], args['n_latents'], args['correlation_loss'], loss_t, args['loss_normalization'], args['n_heads'], args['mask_activation'], results['final_test_loss'], results['best_train_loss'], results['best_train_epoch'], results['model_file']])
    f.close()
...
def save_results(args, model_folder, results):
    ...
    path = os.path.join(model_folder, '..', 'further_test', 'grid_search.csv')
...
def parse_arguments():
    ...
    parser.add_argument('--n_train_epochs', type=int, default=50000)
    parser.add_argument('--n_test_epochs', type=int, default=50000)
...
def main():
    ...
    # create model
    (model, mask_model) = create_model(args)

    model = model.to(args['device'])
    mask_model = mask_model.to(args['device'])
    if args['action'] == 'train':
        # initialize training latents and train model
        train_latents, train_loss, best_loss, best_epoch, best_model, best_mask_model = optimize_network(args, model, mask_model, train_spect, train_pcm, 'train')
        model = best_model.to(args['device'])
        mask_model = best_mask_model.to(args['device'])
        test_loss, final_loss, final_pred, test_latents = optimize_network(args, model, mask_model, test_spect, test_pcm, 'test')
