...
def optimize_network(args, model_low, model_high, spect_low, spect_high, mode, **kwargs):
    ...
    if mode == 'train':
        ...
        optimizers = [net_optimizer_low, net_optimizer_high]
        schedulers = [net_scheduler_low, net_scheduler_high]
    ...
    if mode == 'train':
        ...
        return train_latents_low, train_latents_high, losses, float(best_loss), best_epoch, best_model_low, best_model_high
        ...
        return losses, final_test_loss, all_pred_low, all_pred_high, test_latents_low, test_latents_high
...
def main():
    ...
    # create model
    model_low, model_high = create_model(args)
    model_low = model_low.to(args['device'])
    model_high = model_high.to(args['device'])

    if args['action'] == 'train':
        # initialize training latents and train model
        train_latents_low, train_latents_high, train_loss, best_loss, best_epoch, best_model_low, best_model_high = optimize_network(args, model_low, model_high, train_low, train_high, 'train')
        model_low = best_model_low.to(args['device'])
        model_high = best_model_high.to(args['device'])
        test_loss, final_loss, final_pred_low, final_pred_high, test_latents_low, test_latents_high = optimize_network(args, model_low, model_high, test_low, test_high, 'test')
