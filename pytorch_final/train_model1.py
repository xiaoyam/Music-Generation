...
def sample(train_latents_low, train_latents_high, train_joint_latents
    ...
    generated_low = []
    generated_high = []
    n_points = train_latents_low.shape[0]
    latent_size = args['n_latents']
    time_step = args['time_step']
    for i in range(5):
        idxes = np.arange(n_points)
        low_level = train_joint_latents[idxes[:100]]
...
def save_results(args, model_folder, results):
    ...
    gen_low1D, gen_high1D, gen_low, gen_high = sample(train_latents_low, train_latents_high, train_low_level_latents
	, joint_model_low, joint_model_high, model_low, model_high, args)
...
def parse_arguments():
    ...
    parser.add_argument('--n_train_epochs', type=int, default=6000)
    parser.add_argument('--n_test_epochs', type=int, default=6000)
